...
describe('HTMLTransformer', () => {
  ...
  test('transforms H1 tags', () => {
    expect(transformer.transform('<h1>foo</h1>')).toBe(
      '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 36px">foo</div>'
    );
  });

  test('transforms H2 tags', () => {
    expect(transformer.transform('<h2>foo</h2>')).toBe(
      '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 27px">foo</div>'
    );
  });

  test('transforms H3 tags', () => {
    expect(transformer.transform('<h3>foo</h3>')).toBe(
      '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 21.06px">foo</div>'
    );
  });

  test('transforms H4 tags', () => {
    expect(transformer.transform('<h4>foo</h4>')).toBe(
      '<div style="white-space: pre-wrap; margin: 0; font-weight: bold">foo</div>'
    );
  });

  test('transforms H5 tags', () => {
    expect(transformer.transform('<h5>foo</h5>')).toBe(
      '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 14.94px">foo</div>'
    );
  });

  test('transforms H6 tags', () => {
    expect(transformer.transform('<h6>foo</h6>')).toBe(
      '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 12.06px">foo</div>'
    );
  });
  ...
  test('transforms entire text', () => {
    ...
    expect(transformer.transform(text)).toBe(
      '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 36px">foo</div>' +
        '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 27px">bar</div>' +
        '<div style="white-space: pre-wrap; margin: 0; font-weight: bold; font-size: 21.06px">baz</div>'
    );
  });
});
